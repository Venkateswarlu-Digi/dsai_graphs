    ...
    <div className="shell demand-shell">
      ...
      <main className="main demand-page">
        ...
        <div className="demand-grid demand-grid-top">
          <ChartCard title="Forecast vs Actual — 90d" tag="job_count · weekly · 80% CI" height="sm">
            <ForecastTrend data={graphs.demand_trend_line_all_parts} />
          </ChartCard>
          <ChartCard title="Demand by Category (30d)" tag="predicted_qty" height="sm">
            <DynamicChart
              labels={graphs.demand_by_category_bar.slice(0, 6).map(item => item.category)}
              datasets={[{
                label: 'Predicted Units',
                data: graphs.demand_by_category_bar.slice(0, 6).map(item => item.predicted_units_30d),
        ...
        <div className="demand-grid demand-grid-bottom">
          <ChartCard title="Top Parts — Stock vs Forecast" tag="gap analysis" height="sm">
            <DynamicChart
          ...
          <ChartCard title="Movement Classification" tag="portfolio split" height="sm">
            <DynamicChart
              type="doughnut"
              labels={graphs.movement_class_donut.map(item => item.label)}
              datasets={[{ data: graphs.movement_class_donut.map(item => item.value) }]}
            />
          </ChartCard>
        </div>
